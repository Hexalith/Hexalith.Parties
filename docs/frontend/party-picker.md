...
Selected-party display lookup also has a localized loading label. If a selected-display response arrives after `SelectedPartyId`, `ContextKey`, `AuthContextKey`, token presence, request customizer identity, disabled/read-only state, or picker options change, it is ignored. Hosts must change `AuthContextKey` when an access token rotates without changing the provider/customizer instance. Retry for selected-party transient/unavailable failures re-resolves only the current selected id and current host request context, then returns focus to the selected display region.
