...
- The Ingress uses class `nginx-public` and TLS Secret `hexalith-pages-letsencrypt-tls` in namespace `hexalith-parties`.
- DNS for all three hosts must resolve to the nginx ingress endpoint before live validation.

`publish.ps1` requires the Kubernetes nginx path before it builds and applies workloads: `IngressClass/nginx-public` must exist, the Zot registry must be exposed by `Ingress/zot-ingress` in namespace `zot`, `Service/zot` must be `ClusterIP`, `registry-hexalith-letsencrypt-tls` must exist in `zot`, and `hexalith-pages-letsencrypt-tls` must exist in `hexalith-parties`. A host-level or workstation-local nginx bridge is not accepted for production publish.
