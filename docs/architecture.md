...
> Generated by BMAD *Document Project* (deep scan, 2026-06-02). This is the system-level map. Deep dives live in [data-models.md](data-models.md), [api-contracts.md](api-contracts.md), [source-tree-analysis.md](source-tree-analysis.md), and the hand-maintained event/deployment docs linked throughout.
...
| Orchestration | .NET Aspire (`Aspire.Hosting`, AppHost SDK `13.3.3`) | `13.3.3` / hosting integrations `13.3.5` |
| Actors & pub/sub | `Dapr.Actors` / `Dapr.Client` | `1.18.0-rc02` |
| | `Dapr.AspNetCore` / `Dapr.Actors.AspNetCore` | `1.17.9` |
| | `CommunityToolkit.Aspire.Hosting.Dapr` | `13.3.0-preview` |
...
| API docs | Microsoft.AspNetCore.OpenApi / Swashbuckle.SwaggerUI | `10.0.3` / `10.1.2` |
...
| Testing | xUnit v3 / Shouldly / NSubstitute / bunit / Testcontainers / YamlDotNet | `3.2.2` / `4.3.0` / `5.3.0` / `2.7.2` / `4.10.0` / `18.0.0` |
...
> ⚠️ **Discrepancy:** the README states GDPR features are "not in the MVP" (v1.1), but the crypto-shredding subsystem is implemented and **enabled by default**. The default key store is `LocalDevKeyStorageBackend` (in-memory, dev-only). Reconcile intended production posture with [deployment-security-checklist.md](deployment-security-checklist.md) and provision a real KMS/secret store. (See also [data-models.md §6](data-models.md).)
...
- **Submodules referenced by path** (`Hexalith.EventStore`, `Hexalith.Tenants`) and are **not** checked out in this working copy — initialise root-level submodules before building/running.
- **Discrepancies to reconcile:** GDPR crypto-shredding enabled-by-default vs. README "not in MVP"; MCP exposes **5** tools (`get_party_name_at` does not exist); temporal name-as-of queries are reserved, not implemented.
