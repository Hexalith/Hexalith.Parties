...
- **Projection-index rebuild key mismatch (resolved 2026-06-29):** `PartyIndexProjectionActor` and `ProjectionRebuildService` now both use state key `{tenant}:party-index:default`; regression coverage pins that rebuild and live formulas agree. Detail-projection and manifest keys also agree.
