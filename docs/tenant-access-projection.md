...
Hexalith.Parties consumes Hexalith.Tenants events through DAPR pub/sub and keeps a local tenant access projection in the Parties service process. The projection is updated by the public Tenants client pipeline registered with `AddHexalithTenants()` and exposed to Parties code through `ITenantAccessService`.

## Event Subscription

Parties service maps the Tenants client subscription endpoint at `POST /tenants/events` with the configured `Tenants:PubSubName` and `Tenants:TopicName`. The default topic is `system.tenants.events`.

`Tenants:CommandApiAppId` is consumed by the AppHost / DAPR access-control configuration (Story 11-1), not by Parties service at runtime; the value documents the DAPR app id that the access-control YAML must allow to publish to and subscribe from the Tenants topic.
...
Tenants event consumption is eventually consistent. A just-disabled tenant or just-removed user can be accepted until the matching Tenants event is delivered and processed by the Parties service. Strong synchronous enforcement is outside this projection path unless a separate Tenants/EventStore authorization plugin is enabled on the command gateway.

When the local projection is missing or lagging, check:

- DAPR sidecar health for `parties`.
- The configured pub/sub component and `Tenants:TopicName`.
- Whether `parties` is subscribed to `system.tenants.events`.
