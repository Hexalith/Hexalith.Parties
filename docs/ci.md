...
At the current baseline, the evaluated central catalog selects EventStore 3.103.0, which contains the projection-rebuild APIs consumed by Parties. Package mode remains the authoritative CI and release path; source mode is diagnostic only and must not be used to hide package metadata or publication failures.
