...
> Generated by BMAD *Document Project* (deep scan, 2026-06-02). Annotated map of the repository. Folders are grouped by role; `bin/`, `obj/`, and IDE folders are omitted. **Adopter-facing** = packages consumers normally reference; **Internal** = actor-host private (do not reference from consumer apps).
...
│   │   ├── Events/                    #   ~24 state events + ~18 rejection events (IEventPayload/IRejectionEvent)
...
## Submodules (not checked out in this working copy)

`.gitmodules` declares root-level submodules. The build resolves `Hexalith.EventStore` and `Hexalith.Tenants` as **project references**; `Hexalith.Memories` is optional (rich search). Initialise before building/running:
