...
> Regenerated by BMAD *Document Project* (**full rescan**, deep, 2026-06-02) and reconciled for Epic 8 closure on 2026-08-18. Annotated map of the repository. Folders are grouped by role; `bin/`, `obj/`, and IDE folders are omitted. **Adopter-facing** = packages consumers normally reference; **Internal** = domain-service-host private (do not reference from consumer apps).

```
Hexalith.Parties/
├── Hexalith.Parties.slnx              # Solution (13 projects under `src` + one sample project + 15 runnable tests + 1 support host + e2e)
...
│   │   ├── Program.cs                 #   Add/UseEventStoreDomainService + middleware + subscription endpoints
│   │   ├── Domain/                    #   PartyAggregate and retained GDPR erasure orchestration
│   │   ├── Queries/                   #   SDK IDomainQueryHandler implementations + wire-name shims
...
│   │   ├── HealthChecks/             #   AddPartiesDaprHealthChecks (sidecar/state/pubsub/read models/tenants)
...
│   │   ├── Actors/                    #   event-type resolver only; projection actors retired
│   │   ├── Handlers/                  #   SDK projection handlers + pure event folds
│   │   ├── Services/                  #   read-model eraser and search providers
...
│
├── tests/                             # 15 runnable .NET test projects + 1 support host + Playwright e2e workspace
...
│   ├── Hexalith.Parties.EventStoreGateway.TestHost/ # support host; not a runnable test assembly
...
| Domain service host | `src/Hexalith.Parties/Program.cs` → EventStore-only `/process`, `/query`, `/project*`, `/replay-state`, and rebuild routes |
...
| Projection apply | `src/Hexalith.Parties.Projections/Handlers/` (`IDomainProjectionHandler` + folds) |
