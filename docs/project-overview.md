...
> Regenerated by BMAD *Document Project* (**full rescan**, deep, 2026-06-02) - updated after Epic 1 (`parties-ui`) implementation on 2026-06-10.
...
| **Integration** | Typed .NET client · `parties-ui` Blazor Server BFF · `parties-mcp` (AI tools, 5) · DAPR event subscription · Blazor Picker & AdminPortal |
| **License** | MIT |
...
Single cohesive solution. **Adopter-facing** packages (`Client`, `Contracts`, `Picker`, `AdminPortal`, `Mcp`, `ServiceDefaults`) are what consumers reference; `Hexalith.Parties.UI` is the browser UI/BFF host for this product experience. **Internal** projects (`Hexalith.Parties` host, `Server`, `Projections`, `Security`, `Testing`) are private to the actor host. Sibling submodules `Hexalith.EventStore` and `Hexalith.Tenants` are project references (initialise before building). Annotated tree: [source-tree-analysis.md](source-tree-analysis.md).
