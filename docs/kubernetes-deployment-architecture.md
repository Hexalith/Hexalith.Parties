...
Result (≈5-10 minutes later):
  12 Parties-owned pods running in the hexalith-parties namespace at tag v0.2.0
```
...
| `parties-ui` | 1 | `registry.hexalith.com/parties-ui` | — | Blazor Server browser UI/BFF; calls EventStore SignalR over in-cluster HTTP |
...
| 12 | Verify all 10 generated MinVer image manifests exist in Zot | Exit 6 on missing/unauthorized manifests |
...
Two minutes after step 21, the 12 Parties-owned pods reach `Ready`.
...
This removes:
- All 12 Parties-owned workloads via per-folder `kubectl delete -k`.
...
- **External Ingress**: Only browser UI ingress is provisioned: `eventstore.hexalith.com` -> `eventstore-admin-ui`, `sample.hexalith.com` -> `sample-blazor-ui`, and `parties.hexalith.com` -> `parties-ui`. API/data-path services remain cluster-internal unless an operator adds a separate gateway.
- **Production KMS**: `Parties:CryptoShredding:IsEnabled` is already enabled by default, but the current `LocalDevKeyStorageBackend` is in-memory and dev-only. A production KMS or secret-store-backed key provider is mandatory before real EU PII is processed; until that gate is met, only synthetic data is acceptable.
