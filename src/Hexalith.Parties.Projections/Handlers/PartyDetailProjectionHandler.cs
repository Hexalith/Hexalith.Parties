...
public sealed class PartyDetailProjectionHandler
{
    public static PartyDetail? Apply(string partyId, IEventPayload @event, PartyDetail? state)
    {
        return @event switch
        {
            // The domain has no incarnation token, so an erased identifier is permanent. Treat a
            // later PartyCreated as delayed old-incarnation traffic and preserve the tombstone.
