...
// Publish mode uses the shared JWT Secret generated by deploy/k8s/publish.ps1.
// The cluster-local Keycloak Service is HTTP-only, so it cannot satisfy the
// Tenants production HTTPS OIDC guardrail without a future ingress/TLS story.
const string PublishModeJwtIssuer = "hexalith-dev";
...
_ = WithJwtAuthentication(eventStore, realmUrl, builder.ExecutionContext.IsPublishMode ? PublishModeJwtIssuer : null)
...
_ = WithJwtAuthentication(adminServer, realmUrl, builder.ExecutionContext.IsPublishMode ? PublishModeJwtIssuer : null)
    .WithEnvironment("Authentication__JwtBearer__Audience", "hexalith-eventstore")
    .WithEnvironment("Authentication__JwtBearer__RequireHttpsMetadata", "false")
    .WithEnvironment("Authentication__JwtBearer__SigningKey", "");
...
_ = WithJwtAuthentication(parties, realmUrl, builder.ExecutionContext.IsPublishMode ? PublishModeJwtIssuer : null)
...
_ = WithJwtAuthentication(partiesMcp, realmUrl, builder.ExecutionContext.IsPublishMode ? PublishModeJwtIssuer : null)
...
_ = WithJwtAuthentication(tenants, realmUrl, builder.ExecutionContext.IsPublishMode ? PublishModeJwtIssuer : null)
    ...
    string? publishModeIssuer)
{
    ...
    if (publishModeIssuer is not null)
    {
        return service
            .WithEnvironment("ASPNETCORE_ENVIRONMENT", "Development")
            .WithEnvironment("DOTNET_ENVIRONMENT", "Development")
            .WithEnvironment("Authentication__JwtBearer__Authority", "")
