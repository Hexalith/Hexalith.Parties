...
public class PartyStateTests {
    ...
    public void Apply_IdentifierAdded_AddsToList() {
        ...
        state.Apply(new IdentifierAdded { IdentifierId = "id-1", Type = IdentifierType.VAT, Value = "synthetic-vat-value" });

        state.Identifiers.Count.ShouldBe(1);
        state.Identifiers[0].Id.ShouldBe("id-1");
        state.Identifiers[0].Type.ShouldBe(IdentifierType.VAT);
        state.Identifiers[0].Value.ShouldBe("synthetic-vat-value");
    }
    ...
    [Fact]
    public void Apply_IdentifierRemoved_UnknownId_DoesNotChangeIdentifiers() {
        var state = new PartyState();
        state.Apply(new IdentifierAdded { IdentifierId = "id-1", Type = IdentifierType.VAT, Value = "synthetic-vat-value" });

        state.Apply(new IdentifierRemoved { IdentifierId = "id-missing" });
    ...
    [Theory]
    [InlineData(typeof(AddIdentifier))]
    [InlineData(typeof(IdentifierAdded))]
    [InlineData(typeof(PartyIdentifier))]
    public void IdentifierValueContracts_MarkValueAsPersonalData(Type contractType) {
        ArgumentNullException.ThrowIfNull(contractType);

        System.Reflection.PropertyInfo property = contractType.GetProperty("Value")
            ?? throw new InvalidOperationException($"Expected a Value property on {contractType.FullName}.");

        property.GetCustomAttributes(typeof(PersonalDataAttribute), inherit: true).ShouldNotBeEmpty();
    }
