...
public sealed class HealthEndpointIntegrationTests : IClassFixture<HealthEndpointIntegrationTests.HealthTestFactory>
{
    ...
    [Fact]
    public async Task QueryEndpoint_PubSubDegraded_AddsDegradationHeadersAsync()
    {
        _factory.DetailProjectionActor.GetDetailAsync().Returns(Task.FromResult<PartyDetail?>(CreatePartyDetail("party-1")));
        _factory.DaprClient.CheckHealthAsync(Arg.Any<CancellationToken>()).Returns(true);
        _factory.DaprClient.GetStateAsync<string?>(
            Arg.Any<string>(), Arg.Any<string>(), cancellationToken: Arg.Any<CancellationToken>())
            .Returns((string?)null);
        ...
        HttpResponseMessage response = await client.GetAsync("/api/v1/parties/party-1");

        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        response.Headers.GetValues("X-Service-Degraded").Single().ShouldBe("true");
        response.Headers.Contains("X-Stale-Data-Age").ShouldBeTrue();
    }

    [Fact]
    public async Task QueryEndpoint_AllHealthy_DoesNotAddDegradationHeadersAsync()
    {
        _factory.DetailProjectionActor.GetDetailAsync().Returns(Task.FromResult<PartyDetail?>(CreatePartyDetail("party-1")));
        _factory.DetailProjectionActor.IsRebuildingAsync().Returns(false);
        ConfigureHealthyDaprClient();
        ...
        response.Headers.Contains("X-Stale-Data-Age").ShouldBeFalse();
    }

    [Fact]
    public async Task QueryEndpoint_DetailProjectionRebuilding_AddsDegradationHeadersAndReturns200Async()
    {
        _factory.DetailProjectionActor.IsRebuildingAsync().Returns(true);
        _factory.DetailProjectionActor.GetDetailAsync().Returns(Task.FromResult<PartyDetail?>(null));
        ConfigureHealthyDaprClient();
    ...
    [Fact]
    public async Task ListParties_IndexProjectionRebuilding_AddsDegradationHeadersAsync()
    {
        _factory.IndexProjectionActor.IsRebuildingAsync().Returns(true);
        _factory.IndexProjectionActor.GetEntriesAsync().Returns(Task.FromResult<IReadOnlyDictionary<string, PartyIndexEntry>>(
    ...
    [Fact]
    public async Task WriteEndpoint_StateStoreUnavailable_ReturnsProblemDetailsAsync()
    {
        ConfigureHealthyDaprClient();
