...
`Hexalith.EventStore.SignalR.EventStoreSignalRClient` (`Hexalith.EventStore/src/Hexalith.EventStore.SignalR/EventStoreSignalRClient.cs`) — **`sealed`**, so wrap it behind `IProjectionStream` (Task 2) for testability:
...
- **Test seam precedent:** `Hexalith.EventStore/tests/Hexalith.EventStore.Admin.UI.Tests/TestSignalRClient.cs` wraps the real client with a dummy `HubUrl` + `NullLogger`; here we instead test against the `IProjectionStream` fake (no network at all).
