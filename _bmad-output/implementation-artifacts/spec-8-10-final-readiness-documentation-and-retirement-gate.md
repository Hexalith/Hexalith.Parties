...
- [ ] [Review][Decision] FrontComposer identity for the delivered 8.9/G4 shell slice contradicts between the frozen §4 clause and the matrix — spec §4 clause 1 records gitlink `7a337a21d4ba261bf27aeb3feedde47789f0160a` (`v4.1.1-104-g7a337a21`), packaged `4.1.1`, for the delivered slice; `story-8-3-platform-api-prerequisite-matrix.md`'s "Story 8.10 final retained-identity reconciliation" table records the same delivered slice at gitlink `5cbc5583142a6774ff7813698ad98ec267b336f0` (`v4.3.0-7-g5cbc5583`), packaged `4.3.0`, without distinguishing it as a later, separately-tracked advance. The matrix's own earlier row still cites `7a337a21` too, so the matrix is internally inconsistent as well. The actual checkout and `PlatformApiPrerequisitesTests.cs`'s `FrontComposerSha` constant both match `5cbc5583`. Fixing the spec's own clause requires editing frozen `<frozen-after-approval>` text — needs your call on which identity is authoritative and whether the frozen clause may be amended. [spec-8-10-final-readiness-documentation-and-retirement-gate.md:58-59, story-8-3-platform-api-prerequisite-matrix.md:66,101]
- [ ] [Review][Decision] PolymorphicSerializations gitlink advanced past every closure/tracking artifact — the submodule is now at `8aeed1d27c9a050bc4bec6d89051aa00de306a69`, but `test-summary.md`'s `polymorphicserializations-stylecop-fix-incomplete-at-selected-gitlink`/`authorized-owner-fixes-not-immutable` blockers, the 8-3 matrix's G5 row, and `.gitlink-signoff.tsv` all still name the superseded `0dca9e9d3f8b2a20ba426b84fa575ab4e7b5562b`. No fitness test pins a specific PolySer SHA, so nothing fails on this, but no artifact says whether `8aeed1d2` even builds clean. Needs a human call: accept `8aeed1d2` (reconcile every artifact and confirm a clean build), or roll back to the last validated `0dca9e9d`. [_bmad-output/implementation-artifacts/tests/test-summary.md:615, story-8-3-platform-api-prerequisite-matrix.md:68]
- [ ] [Review][Decision] Commons gitlink advanced with no `.gitlink-signoff.tsv` entry — `references/Hexalith.Commons` moved to `6da79aed2daa4e199689331ee3196f7872c0988a` (reflected in the matrix and `PlatformApiPrerequisitesTests.cs`'s `CommonsSha`), but the 2026-09-05 `.gitlink-signoff.tsv` block added rows for Builds/EventStore/FrontComposer/Memories/Tenants and omitted Commons. `scripts/gitlink-rc-gate.sh` will fail closed at release-tag time for lack of sign-off; needs an actual approver decision, not a fabricated row. [.gitlink-signoff.tsv]
- [ ] [Review][Decision] sprint-status.yaml status comment contradicts the value it precedes — the Story 8.10 comment ends "...Status moved review -> in-progress because re-measurement... found the Release solution build RED... Two blockers are open..." but the line immediately below sets the key to `review`, not `in-progress`. Needs a human call on which is actually true right now given the open blockers the comment itself names. [_bmad-output/implementation-artifacts/sprint-status.yaml:216-224]
