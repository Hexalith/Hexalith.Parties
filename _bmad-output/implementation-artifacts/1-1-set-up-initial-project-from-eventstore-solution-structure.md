...
- [x] [Review][Decision] Brittle pinned 22-package transitive list — resolved 2026-05-16 via option (a): dropped the `expectedTransitivePackageNames` HashSet and the `transitivePackageNames.ShouldBe(...)` exact-set check. The remaining `violations.ShouldBeEmpty` check (prefix `Microsoft.Extensions.` + `allowedContractInfrastructurePackages` allow-list) preserves the architectural guard against direct Dapr/Server/Projections/etc. leakage while no longer failing on benign NuGet patch bumps that add or remove a single transitive. Test still passes. [tests/Hexalith.Parties.Client.Tests/FitnessTests/ClientArchitecturalFitnessTests.cs:387-411]
...
- 2026-05-16: Adjudicated brittle-pin Decision via option (a): removed the exact-set `transitivePackageNames.ShouldBe(...)` assertion and its 22-name `expectedTransitivePackageNames` HashSet. The prefix + allow-list `violations.ShouldBeEmpty` check remains as the sole architectural guard.
- 2026-05-16: Second code-review pass (Blind Hunter, Edge Case Hunter, Acceptance Auditor) found zero new findings. Applied 4 of the 5 open prior Review Findings: targeted hosted-service removal (preserving `EventStoreAuthorizationStartupValidator`), customMessage rewording on both Client fitness assertions, dropped the unused `client` local in `DomainServiceResolver_PartyWildcardRegistration_PointsToPartiesProcessAsync`, and committed the `Hexalith.EventStore` submodule pointer.
