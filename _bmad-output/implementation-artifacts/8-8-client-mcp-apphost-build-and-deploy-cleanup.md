...
revalidated: 2026-07-31T15:45:08+02:00
source_status: backlog
target_status: blocked
baseline_commit_at_story_start: 8644b1b1a50d2f1ab7f1cdad8dc00a88314080e2
baseline_commit_at_revalidation: ed007f81878bb4f32bab5a56f5bd7978ed738f80
ai_tools_root_pin_at_revalidation: 859d53b792e1db9562b0ff66dbc335f418a688be
ai_tools_checkout_at_revalidation: 859d53b792e1db9562b0ff66dbc335f418a688be
builds_root_pin_at_revalidation: b529b665a6f076d07d218266ab74ca211f34f5a7
builds_checkout_at_revalidation: b529b665a6f076d07d218266ab74ca211f34f5a7
...
polymorphic_serializations_root_pin_at_revalidation: f3b23304283b0e7a35ffa66bf8d9bf2499e35e66
polymorphic_serializations_checkout_at_revalidation: f3b23304283b0e7a35ffa66bf8d9bf2499e35e66
tenants_root_pin_at_revalidation: 2cd7edf5088c54bbeced37d2f8164c36889b7cac
tenants_checkout_at_revalidation: 2cd7edf5088c54bbeced37d2f8164c36889b7cac
---
...
1. Given Story 8.6 is `done` and Story 8.7 remains `blocked` in the authoritative `8.6 -> 8.7 -> 8.8` sequence, when Story 8.8 is prepared or owner-side prerequisite work proceeds, then no Parties production migration or deletion begins until 8.7 completes (or an approved architecture/product artifact explicitly changes the sequence) and each 8.8 slice’s Story 8.3 row is consumable.
...
- [ ] Clear the story and slice start gates before production edits (AC: 1, 2, 12)
  ...
  - [x] 2026-09-06: Story 8.6 is `done`; keep 8.8 `blocked` on Story 8.7 plus each slice’s Story 8.3 row.
  ...
  - [x] Confirm the Builds row does not match the revalidated root/checkout `b529b66...` identity; its only delta from the prior `e85a319...` snapshot is a Builds-internal implementation-artifact edit, not new shared build-surface or consumer-parity evidence; halt the build slice.
  - [x] Confirm EventStore Story 1.20 delivers and authorizes G6 only at exact source `fa2d1c9...` or its proof artifacts; the current `v3.86.0`/`e4618d9...` identity has no relevant G6 source delta and is publicly packaged, but the packet forbids identity substitution. Obtain explicit descendant/release acceptance or select the approved identity, then complete Parties parity; do not request another additive G6 API.
  - [x] Confirm all eight root-declared submodule checkouts match their root gitlinks and are clean; do not treat synchronization alone as owner approval or consumer parity.
  - [x] Run `npx --no-install commitlint --last --verbose` against baseline `ed007f8...` and record its failure (`type-empty`, `subject-empty`, and `body-max-line-length`) as a release-policy blocker; do not rewrite pushed `main` from this story.
  ...
  - [ ] Complete Story 8.7 or record an approved sequence change (Story 8.6 is `done`).
...
- [ ] Adopt Commons HTTP and owner-delivered EventStore client mechanics without duplicating them (AC: 2-4, 12)
  ...
  - [ ] Treat the current `BoundedProblemDetailsReader` as unbounded for 8.8: it parses the response stream without byte, JSON-depth, or string-length limits. Keep the Parties bounded adapter or require an additive owner API; the class name is not parity evidence.
...
- [ ] Adopt the exact approved Builds surface without weakening repository gates (AC: 2, 11, 12)
  ...
  - [ ] Stage imports/adapters first and retain local probes until Debug source-mode restore/build/test parity, Release package-mode restore/build/test/pack/release parity, and rollback pass.
...
- [ ] Disposition the G1/G2 degraded-response and DAPR-health planning overlap without broadening scope (AC: 2, 12)
  - [ ] Reconcile the Story 8.3 matrix/router assignment of G1/G2 to 8.8 with the draft 8.8 spec, which omits those rows from its start gates and file map; require an explicit architecture/product decision before adoption or deferral.
  - [ ] If retained in 8.8, select an exact EventStore identity and prove full middleware/header, check-name/tag, state-store/pub-sub/config-store, failure-status, timeout, and rollback parity. The visible `AddEventStoreDaprHealthChecks` API alone is insufficient.
  - [ ] If reassigned, record the approved owner/story in the matrix and explicitly exclude G1/G2 deletion from this story.
...
- The approved sequence remains `8.6 -> 8.7 -> 8.8`. Story 8.6 is `done`; Story 8.7 remains `blocked`, so owner work may proceed but Parties source consumption remains sequence-gated.
- Root and `origin/main` both select baseline `ed007f8...`, whose current message fails `npx --no-install commitlint --last --verbose` with `type-empty`, `subject-empty`, and `body-max-line-length`. This is an owner-governed release-policy blocker: do not amend or force-rewrite pushed `main` in this story, and do not tag/release until the repository's approved Git governance dispositions it and direct-main protection is verified.
...
- EventStore Story 1.20 is now `done` with an owner-approved `available` packet dated 2026-07-26. It authorizes migration only to exact source `fa2d1c9...`, proof package `999.1.20-proof.fa2d1c9910f8`, or the digest-pinned container recorded there. The current Parties EventStore root/checkout `e4618d9...` (`v3.86.0`) is a descendant and has no source delta from `fa2d1c9...` across the relevant G6 gateway/client/contracts files, but it is not an approved exact identity. Story 8.6 is `done`; Story 8.8 stays sequence-gated on 8.7, and G6 consumption still requires selected-identity acceptance.
- G6 API delivery is complete at the exact approved Story 1.20 identity: typed command/query results, all six lifecycle states, `200`/`304` metadata, typed gateway failures, and gateway/DAPR registration exist with producer tests. Reclassify the stale matrix row from `needs-additive-api` to available-at-approved-identity before consumption; the remaining work is selected-identity acceptance, Parties adapter parity, and rollback, not another additive owner API. G7/G9 delivery remains absent: the tenant claim constant is not in the approved public Contracts surface, and `AggregateIdentity.IsValid(string)` and `UniqueIdHelper.IsValidUlid(string)` do not exist at the current pins.
- G8 is partial but unclosed: EventStore has audience-aware `WithJwtBearerSecurity`, generic `AddEventStoreGatewayClient`, and DAPR service-invocation wiring, but no owner-approved publish/no-secret parity and no independently selectable command/query/GDPR module clients. The current FrontComposer AppHost composes EventStore, Tenants, Parties, and `frontcomposer-ui`. Memories exposes the candidate `AddHexalithMemoriesSearchIndexServer(...)` Aspire helper, but FrontComposer does not consume it. No owner-approved packet yet proves explicit `parties-mcp`, standalone `parties-ui` disposition, optional Memories/sample parity, supported publish targets, or rollback.
...
- Commons HTTP and Builds no longer match the historical `available` identities in the matrix: current selections are Commons `2.29.0`/`f2b5f1b...` and Builds `b529b66...` (`v4.23.0-18-gb529b66`). The latest Builds delta changes only its Story 6.1 preservation-contract record; it does not refresh the 8.8 API receipt or consumer proof. Each row must be refreshed or its older exact identity deliberately selected before consumption.
- G12 publication selection is resolved, and the selected Commons `2.29.0` and Tenants `5.3.0` packages are published. Its recorded package-only Parties proof still used Commons `2.28.0` and Tenants `2.4.2`; the current Tenants root/checkout is `2cd7edf...` (`v5.3.0-3-g2cd7edf5`). That delta contains tenant UI/query lifecycle, accessibility/localization, tests, and submodule-pointer work, not a new G7/G9 receipt or current-version Parties parity. Rerun package-only restore/build and consumer parity before deletion without reopening the publication decision merely because versions advanced.
...
- The Story 8.3 G1/G2 degraded-response/Dapr-health row and owner router assign the work to 8.8, but the draft 8.8 spec does not include that row in its start gates or file map. Keep `DegradedResponseMiddleware` and Parties Dapr health checks local until architecture/product owners explicitly reconcile adoption versus deferral; do not silently delete them under “platform runtime concerns.”
- G2 has partial packaged delivery at EventStore 3.86.0 through `AddEventStoreDaprHealthChecks` and five public Dapr checks. It still differs from Parties check names/tags, failure statuses, optional checks, and product-specific readiness composition. G1 remains absent: EventStore has no replacement for the Parties GET-only degraded/stale response headers. Keep both local until the matrix records a narrow adapter/owner disposition plus exact parity.
...
- Current Commons HTTP registration, correlation, and paging primitives are viable candidates, but `BoundedProblemDetailsReader` performs an unbounded stream parse with default JSON limits. Do not delete Parties byte/depth/string bounds unless an additive owner surface or a retained compatibility adapter proves the same limits.
...
|---|---|---|---|
| Commons HTTP | `src/Hexalith.Parties.Client/Extensions/PartiesClientServiceCollectionExtensions.cs`, `PartiesClientOptions.cs`, `PartiesClientException.cs`, `Hexalith.Parties.Client.csproj`; the matching dependency-injection, exception, package, and fitness tests under `tests/Hexalith.Parties.Client.Tests` | Keep Parties routes, domain mapping, public interfaces, typed command/query clients, and the truly bounded error adapter; replace only selected registration, endpoint validation, correlation, and paging mechanics until Commons supplies equivalent byte/depth/string bounds | Exact Commons identity/API inventory, producer tests, source/package consumer builds, invalid URI/tenant cases, independent typed-client composition, cancellation, correlation, adversarial oversized/deep/long error input, and rollback |
...
- At final revalidation the root worktree was clean and all eight root-declared submodule checkouts were clean and matched their root gitlinks. Relative to the prior snapshot, only Builds (`b529b66...`) and Tenants (`2cd7edf...`) moved; EventStore, Commons, FrontComposer, and Memories did not. Revalidate immediately before implementation because synchronization alone neither selects the EventStore proof identity nor supplies owner/consumer parity.
...
Build sequentially. Use `Debug` for source/project-reference validation and `Release` for package/CI validation. Run the relevant direct xUnit v3 executables after each build, then the repository lanes:

```bash
git ls-tree HEAD references/Hexalith.AI.Tools references/Hexalith.Builds references/Hexalith.Commons references/Hexalith.EventStore references/Hexalith.FrontComposer references/Hexalith.Memories references/Hexalith.PolymorphicSerializations references/Hexalith.Tenants
git submodule status -- references/Hexalith.AI.Tools references/Hexalith.Builds references/Hexalith.Commons references/Hexalith.EventStore references/Hexalith.FrontComposer references/Hexalith.Memories references/Hexalith.PolymorphicSerializations references/Hexalith.Tenants
...
dotnet build Hexalith.Parties.slnx -c Debug -m:1 -p:UseHexalithProjectReferences=true -p:UseNuGetDeps=false -p:NuGetAudit=false -p:HexalithPartiesPackageVersion=1.0.0 --verbosity minimal
dotnet ./tests/Hexalith.Parties.Client.Tests/bin/Debug/net10.0/Hexalith.Parties.Client.Tests.dll
dotnet ./tests/Hexalith.Parties.Mcp.Tests/bin/Debug/net10.0/Hexalith.Parties.Mcp.Tests.dll
dotnet ./tests/Hexalith.Parties.AdminPortal.Tests/bin/Debug/net10.0/Hexalith.Parties.AdminPortal.Tests.dll
dotnet ./tests/Hexalith.Parties.Authentication.Tests/bin/Debug/net10.0/Hexalith.Parties.Authentication.Tests.dll
dotnet ./tests/Hexalith.Parties.Tests/bin/Debug/net10.0/Hexalith.Parties.Tests.dll
dotnet ./tests/Hexalith.Parties.Ci.Tests/bin/Debug/net10.0/Hexalith.Parties.Ci.Tests.dll
...
python3 scripts/pack-release-packages.py artifacts/packages 1.0.0
python3 scripts/validate-nuget-packages.py artifacts/packages
python3 scripts/validate-consumer-package-references.py artifacts/packages --work-directory artifacts/consumer-package-smoke
...
- Checklist corrections applied and revalidated on 2026-07-31: kept the story honestly blocked; refreshed the root baseline and all eight root-declared gitlink/checkout identities after the latest synchronization; distinguished the exact approved EventStore identity from its unchanged current descendant; recorded the baseline commit-message policy failure without rewriting pushed history; split the work into twelve independent evidence gates; added an exact per-gate change/retention/proof map; prevented synchronized gitlinks or visible APIs from authorizing deletion; preserved tool/domain and full AppHost rollback ownership; added MCP token-passthrough/header-chain defenses and `200`/`304` lifecycle parity; reconciled the current semantic-release version path and Story 8.6 status metadata; expanded atomic retirement inventory; and required post-switch rollback plus exact evidence per deletion.
...
- 2026-07-31T15:30:00+02:00 - Revalidated root `ed007f8...` after the latest submodule synchronization: all eight root-declared checkouts are clean and match their gitlinks; Builds advanced to `b529b66...` and Tenants to `2cd7edf...`, while EventStore, Commons, FrontComposer, and Memories are unchanged. The new Builds/Tenants deltas do not deliver or approve the open 8.8 owner surfaces.
- 2026-07-31T15:30:00+02:00 - Reclassified G6 as delivered/available only at the exact Story 1.20 identity, recorded packaged partial G2 delivery, retained G1, G5, G7/G9, G8, and G11 gates, and rejected the current Commons `BoundedProblemDetailsReader` as true byte/depth/string-bound parity.
- 2026-07-31T15:45:08+02:00 - `npx --no-install commitlint --last --verbose` failed for baseline `ed007f8...` with `type-empty`, `subject-empty`, and `body-max-line-length`; recorded the pushed-main release-policy blocker for owner-governed disposition without amending history.
- 2026-09-06T16:34:00+02:00 - Course correction: Story 8.6 is `done`. Story 8.8 stays `blocked` on Story 8.7 plus each slice’s Story 8.3 row. Frozen key remains `8-8-client-mcp-apphost-build-and-deploy-cleanup`.

### Completion Notes List

- Story 8.8 is context-complete as a guarded packet but remains blocked for production work by Story 8.7 and slice-specific owner/identity/parity prerequisites. Story 8.6 is `done`.
- EventStore G6 owner delivery is approved, but its exact approved identity is not selected by Parties; Commons HTTP and Builds also require matrix refresh against current identities and facet-level consumer parity.
- G5, G7/G9, G8, and G11 still require delivered/approved surfaces plus exact identities, Parties parity, and exercised rollback. Updating every root submodule removed checkout drift but did not close those evidence gates.
- No product tests were run during this documentation-only revalidation because no production code changed. YAML/front-matter, repository-state, Markdown integrity, link, File List, and diff checks are the applicable validation; the baseline commitlint gate is explicitly failing as recorded above.
