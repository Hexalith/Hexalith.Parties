...
- [ ] [Review][Patch] McpSessionScope is copy-pasted across three test files — extract a single shared utility [tests/Hexalith.Parties.CommandApi.Tests]
...
- [ ] [Review][Patch] `_detailsByActorId`/`_indexByActorId` not cleared between tests; `SetTenantParties` silently overwrites duplicate ids; `ResetIndexProxy` not synchronized [tests/Hexalith.Parties.CommandApi.Tests/Controllers/PartiesControllerProblemDetailsTests.cs:911-950]
...
- [ ] [Review][Patch] Stringly-typed substring assertions on `InvalidOperationException.Message` — throw a typed access-denied exception with reason-code property [tests/Hexalith.Parties.CommandApi.Tests/Mcp/McpToolTenantAuthorizationTests.cs:1216,1235,1254]
- [ ] [Review][Patch] McpToolTenantAuthorizationTests builds a ServiceProvider with only `ITenantAccessService` — confusing failure mode if tool resolves more services [tests/Hexalith.Parties.CommandApi.Tests/Mcp/McpToolTenantAuthorizationTests.cs:1261-1283]
...
- [ ] [Review][Patch] `ShouldNotContain("tenant-b-party")` substring check fails on partial leakage in links/metadata — parse JSON and assert array empty [tests/Hexalith.Parties.CommandApi.Tests/Controllers/CrossTenantIsolationTests.cs:710-713]
...
- [ ] [Review][Patch] `RemoveUserFromTenant` test brittle to "cannot remove last owner" upstream policy — seed a second owner before removal [tests/Hexalith.Parties.CommandApi.Tests/Authorization/HelperDrivenTenantAccessTests.cs:494-501]
...
- [ ] [Review][Patch] `TenantsBackedAccessE2ETests` silently no-ops when fixture unavailable — use `SkippableFact`/`Skip.If` [tests/Hexalith.Parties.IntegrationTests/Tenants/TenantsBackedAccessE2ETests.cs:2123-2127,2147-2150,2173-2176]
...
- [ ] [Review][Patch] `subscriptionScopes` missing → `Warn` instead of `Fail` (left for separate change — production-vs-local-dev gating logic in Test-PubSub is broader than Story 11.4 scope) when production scoping is expected [deploy/validate-deployment.ps1:250-262]
...
- [ ] [Review][Patch] `Get-YamlKind` does not strip surrounding quotes — `kind: "Component"` slips through [deploy/validate-deployment.ps1:248]
- [ ] [Review][Patch] `Test-TopicAllowedForApp` whitespace/quoting fragility — trim both sides and strip quoted values [deploy/validate-deployment.ps1:119-131]
- [ ] [Review][Patch] Multi-document `pubsub*.yaml` files only have first document inspected — split on `^---` and iterate [deploy/validate-deployment.ps1:245]
- [ ] [Review][Patch] `WriteLocalDevConfig` writes both subscription and integration files so the warn-only branch is never exercised — drop one or assert different signal [tests/Hexalith.Parties.DeployValidation.Tests/TenantsDeploymentValidationTests.cs:1577-1587,1811-1812]
...
- [ ] [Review][Patch] `subscription-tenants.yaml` declares `deadLetterTopic` but no retry/`maxDeliveries` resiliency policy — add (spec line 175) [deploy/dapr/subscription-tenants.yaml:74-91]
- [ ] [Review][Patch] `subscription-tenants.yaml` `metadata.name: tenants-events-commandapi` not unique-namespaced — collision risk in shared cluster [deploy/dapr/subscription-tenants.yaml:82]
...
- [ ] [Review][Patch] Setup-order step 4 "Confirm Parties is subscribed to `system.tenants.events`" has no actionable verification — add a `dapr` or `curl` command [docs/getting-started.md:397-404]
- [ ] [Review][Patch] Getting-started still presents Keycloak realm-role guidance as authoritative in legacy sections — remove or qualify so Tenants membership/role is the single source of truth [docs/getting-started.md]
- [ ] [Review][Patch] README adds Party Picker link unrelated to story 11.4 (carried in commit 12bbac3 mixed with story 10-3) — move to story 10-3 file list or accept as cross-story commit [README.md:17]
- [ ] [Review][Patch] Magic actor-id format string `"{tenantId}:party-detail:{detail.Id}"` duplicated across multiple test files — extract a `TenantActorIds` helper [tests/Hexalith.Parties.CommandApi.Tests/Controllers/PartiesControllerProblemDetailsTests.cs:898,913,928,941,965; CrossTenantIsolationTests.cs:795,824]
