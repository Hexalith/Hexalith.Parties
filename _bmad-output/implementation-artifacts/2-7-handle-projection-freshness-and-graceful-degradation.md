...
- 2026-05-21: Story-automator review (cycle 2) — verified cycle-1 fixes still in place. Fixed M3 (consolidated four duplicated private `Freshness(...)` factories with diverging signatures into a single public static `ProjectionFreshnessMetadata.Create(status, params warningCodes)` so a future freshness-status call site cannot reintroduce a divergent private helper alongside the constants). Updated `ContractsPublicApiSnapshot.txt` for the additive public `Create` method. Story remains `done`. Validation: `dotnet build` clean across Contracts, Projections, and Parties; `dotnet test` green for the targeted story-2.7 suites (90/90 in Parties.Tests filter, 139/139 in Projections.Tests, 81/81 in Contracts.Tests, 24/24 in HttpPartiesQueryClientTests).
...
- **MEDIUM M3 (cycle 2) — `Freshness(...)` factory duplicated across four files with two diverging signatures.** `PartyDetailProjectionActor.cs`, `PartyIndexProjectionActor.cs`, `PartyDetailProjectionActorExtensions.cs`, and `PartyIndexProjectionQueryActor.cs` each declared a private static factory; the actor sites used `(status, params string[] warningCodes)` while the extension/query-actor sites used `(status, string? warningCode)`. Cycle-1's M2 fix consolidated only the warning-code string literals — a new freshness-status site would still have invented its own private helper. **Fix applied:** added `public static ProjectionFreshnessMetadata.Create(ProjectionFreshnessStatus status, params string[] warningCodes)` next to the warning-code constants and removed all four private duplicates. Updated `ContractsPublicApiSnapshot.txt` for the additive public method.
- **LOW L2 (cycle 2) — `DegradedResponseMiddleware._degradedSince` lazy-init race.** `_degradedSince ??= DateTimeOffset.UtcNow` is not atomic across concurrent first-degraded requests; the drift is bounded by the request inter-arrival window (typically sub-millisecond) and is observable only as a tiny stale-age skew on the very first degraded response. Not fixed: the cost of `Interlocked.CompareExchange<DateTimeOffset?>` exceeds the impact.
- **LOW L3 (cycle 2) — `PartyDetailProjectionQueryActor.QueryAsync` collapses "party not found" into `ActorNotFoundInfrastructure`.** A current/healthy projection returning `Detail == null` (party never existed) reports as infrastructure failure rather than a logical not-found. Pre-existing from story 2.3; not a story-2.7 regression. Out of scope for this review.
