...
4. **MVP search does not claim unavailable fields**
   ...
   - Then the public result does not report those fields as matched,
...
| AC4 | Tests prove MVP public search results do not emit `email`, `contactChannel`, `identifier`, `semantic`, `graph`, or duplicate-detection match metadata unless a later accepted contract explicitly enables that mode. |
...
| Authorized tenant with prefix or contains display-name matches | Success with match metadata identifying `displayName` and the implemented match type; ordering is deterministic. |
...
| Email/contact/identifier-only match | Not reported as an MVP public match; reserved for later accepted search-model work. |
...
| Stale/rebuilding/degraded index with safe cached entries | Preserve accepted bounded local-only/degraded behavior only when current-tenant provenance and erasure filtering are proven. |
...
- [ ] Task 2: Reconcile display-name-only MVP search behavior (AC: 1, 2, 3, 4)
  ...
  - [ ] Rank exact above prefix, prefix above contains, and any retained fuzzy match below deterministic lexical matches. Use a stable tie-breaker such as display name then party id.
...
- [ ] Task 6: Add or harden focused tests (AC: 1-7)
  ...
  - [ ] Extend `LocalFuzzyPartySearchProviderTests` or add a display-name-specific provider test suite proving exact, prefix, contains, optional fuzzy, deterministic ranking, tie-breaking, pagination, empty query, and no future-reserved fields in MVP mode.
...
| Optional fuzzy display-name match | If retained, tests prove it applies only to display names, is ranked below exact/prefix/contains, ignores digit-heavy false positives, and emits bounded metadata. |
| Future-reserved fields | Tests prove MVP public results do not emit `email`, `contactChannel`, `identifier`, `semantic`, `graph`, duplicate, or Memories-only match metadata. |
...
- `LocalFuzzyPartySearchProvider` currently evaluates display names, contact channels, identifiers, and type text, and can emit field names such as `email`, `contactChannel`, `identifier`, and `type`. That is broader than Story 2.5's MVP display-name public contract. Reconcile this deliberately rather than allowing accidental future-search metadata through the MVP path.
...
- Whether fuzzy display-name matching is part of the MVP public contract remains an implementation/product decision if not already accepted; exact, prefix, and contains matching are required by this story.
