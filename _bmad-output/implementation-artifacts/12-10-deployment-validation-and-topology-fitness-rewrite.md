...
_Code review performed 2026-05-13 (bmad-code-review). 3 decisions resolved, 12 patches applied, 5 deferred, ~14 dismissed. Deploy-validation tests 41/41 (+1 new failure-path JSON sanitization test), AppHost topology + architectural fitness 29/29, full Release solution build clean. See `_bmad-output/implementation-artifacts/deferred-work.md` for deferred items._

#### Decisions

- [x] [Review][Decision] xUnit v3 migration scope creep [`Directory.Packages.props`, `tests/Directory.Build.props`, all 11 test csproj files, `PartiesAspireTopologyFixture.cs`] — Story 12.10 Technical Constraints (Dev Notes line 151) lists `xUnit 2.9.3` as the required version. The commit bumps to xunit.v3 and adds `<NoWarn>$(NoWarn);xUnit1051</NoWarn>` in `tests/Directory.Build.props`, blanket-suppressing the v3 cancellation-token analyzer across every test project. Justified in side proposal `sprint-change-proposal-2026-05-12-xunit-v3.md` but no AC sanctions it. Decide: keep (intentional, side-proposal accepted), revert, or scope to a follow-up story.
- [x] [Review][Decision] Tenants receiving sidecar grants `eventstore -> POST /**` [`deploy/dapr/accesscontrol.tenants.yaml:14-21`] — AC3 / story task line 63-64 ("Tenants access control exposes only the accepted readiness/invocation paths and does not grant broad caller sets"). The YAML allows eventstore to POST to ANY path on the Tenants sidecar with a wildcard `/**`. The validator never flags this because the wildcard check only runs against `accesscontrol.parties.yaml`. Either (a) tighten the YAML to specific paths eventstore actually invokes on Tenants, or (b) document that eventstore's broad access is intentional and add a validator rule confirming the documented set. EventStore uses pubsub (component-level) for `system.tenants.events`, not service-invocation, so the rationale for `POST /**` is unclear from the diff.
- [x] [Review][Decision] Coverage Matrix claims `AppHostDaprTopologyValidationTests` updates that the diff does not show [story file Coverage Matrix row 3] — Matrix says action taken was "added production validator split-file checks" but `AppHostDaprTopologyValidationTests.cs` is NOT in the File List and not in the diff. The production validator additions actually live in `validate-deployment.ps1` and the two `*DeploymentValidationTests.cs` files. Decide: rewrite the matrix row to clarify ("no changes; production validator owns equivalent matrix"), or add the missing tests.

#### Patches

- [x] [Review][Patch] `Test-InvocationPolicy` cross-policy false positives [`deploy/validate-deployment.ps1:219-230`] — Non-greedy `.*?` between `appId`, `operations`, `name`, `httpVerb`, `action` segments has no `(?!^\s*-\s*appId:)` lookahead, so a regex spanning across multiple `- appId:` blocks in the same file can produce a false positive (e.g., `parties` having no operations but matching `name: /ready` from an unrelated `- appId: eventstore` block). Scope the match to a single policy block, or replace with structured YAML parsing using YamlDotNet (already a package reference in the test project).
- [x] [Review][Patch] Domain-route check uses 4 independent disjoint regex matches [`deploy/validate-deployment.ps1:1031-1034`] — `$content -match '\*\|party\|v1'`, `appId: parties`, `methodName: process`, `domain: party` are each checked independently with `-and`. They are not required to belong to the same `domainServices` list element. A topology.yaml where these values appear in unrelated stanzas (e.g., `appId: parties` in `metadata.labels`) passes the gate. Anchor to a single `domainServices` entry using a multi-line block match `(?s)-\s+key:\s*['"]?\*\|party\|v1['"]?[^-]*?appId:\s*parties[^-]*?methodName:\s*process[^-]*?domain:\s*party`, or parse with YamlDotNet.
- [x] [Review][Patch] Admin UI wiring regex not anchored under `eventStoreAdminUi` parent [`deploy/validate-deployment.ps1:1042`] — `(?m)^\s*adminServerAppId:\s*eventstore-admin\s*$` matches a stray top-level key. Anchor under the `eventStoreAdminUi:` parent or parse structurally.
- [x] [Review][Patch] Topology regex assertions don't handle quoted scalars [`deploy/validate-deployment.ps1:1031-1034,1042,216`] — `appId: "parties"` (idiomatic YAML for strings) fails the regex `^\s*appId:\s*parties\s*$`. Add optional quote handling: `(?m)^\s*appId:\s*['"]?parties['"]?\s*$`. Same for `methodName`, `domain`, `adminServerAppId`, and `Test-AppIdListed`'s `^\s*-\s*$escaped\s*$` (block-style only — should also accept quoted forms).
- [x] [Review][Patch] Admin sidecar `policies: []` literal coupling [`deploy/validate-deployment.ps1:402`] — `$adminContent -match "policies:\s*\[\]"` requires exact flow-style. A YAML file with `policies:` followed by no children (which parses as null — semantically the same locked-down state) FAILS the check. Accept either form: `policies:\s*\[\s*\]|policies:\s*(?:#|$|\r?\n\s*\w+:)` (policies followed by null or by an unrelated sibling key).
- [x] [Review][Patch] Parties wildcard check matches comments [`deploy/validate-deployment.ps1:360`] — `$partiesContent -match "name:\s*['"]?/\*\*['"]?"` matches a comment line like `# name: /**` or `# Allow eventstore /** during migration`. Strip comments before the regex: `$partiesContentNoComments = $partiesContent -replace "(?m)^\s*#.*$", ""`.
- [x] [Review][Patch] Production `publishingScopes` ships with literal `eventstore=sample.parties.events` [`deploy/dapr/pubsub-kafka.yaml:72`, `pubsub-rabbitmq.yaml`, `pubsub-servicebus.yaml`] — The literal sample tenant `sample.` is a developer-template value baked into the production manifest. The comment block says "MUST CUSTOMIZE: Replace example topic names" but the directive applies only to `subscriptionScopes`. Either remove the `eventstore=` entry (DAPR treats absence as unrestricted publishing — matches the architecture intent for the EventStore gateway) or replace with `eventstore={env:EVENTSTORE_PUBLISH_TOPICS}`.
- [x] [Review][Patch] `ExtractProjectResources` `ToDictionary` crashes on duplicate resource names [`tests/Hexalith.Parties.Tests/FitnessTests/AppHostTenantsTopologyTests.cs:~204-213`] — Duplicate `AddProject<...>("parties")` calls (e.g., under `#if DEBUG` branches) throw `ArgumentException: An item with the same key has already been added` instead of producing a clear assertion failure. Use `.GroupBy(g => g.Key).ToDictionary(g => g.Key, g => g.First().Value)` and assert no duplicate group sizes greater than 1.
- [x] [Review][Patch] `AppHostProgramUsesWaitForForDependencyReadiness` matches comments [`tests/Hexalith.Parties.Tests/FitnessTests/AppHostTenantsTopologyTests.cs:~182-192`] — `program.ShouldNotContain(".WaitForStart(")` flags a comment like `// .WaitForStart(` or a disabled `#if` block. Strip C# comments before the assertion: parse and remove `//.*$` and `/*...*/` blocks, or use a regex-based assertion that requires `.WaitForStart(` to be preceded by an identifier/method-call token.
- [x] [Review][Patch] Failure-path tests lack paired JSON output sanitization assertions [`tests/Hexalith.Parties.DeployValidation.Tests/DeploymentValidationTests.cs:~796-822`] — Sanitized-output rule requires every NEW validator failure path to have paired console+JSON sanitization assertions. The new negative tests `MissingEventStoreTopologyResource_FailsWithGatewayRecommendation` and `PartiesMcpEnabledButMissingResource_FailsWithOptionalMcpRecommendation` only assert console output. Add a `jsonOutput: true` variant that asserts the same redaction guarantees. The only paired test (`TopologyValidation_Output_DoesNotLeakSecretLookingManifestValues`) is a passing-case test (`exitCode.ShouldBe(0)`).
- [x] [Review][Patch] `Test-ScopesContainAll` case-insensitivity admits wrong-cased app-ids [`deploy/validate-deployment.ps1:200-208`] — `$Scopes -notcontains $requiredScope` is case-insensitive by default. DAPR app-ids are case-sensitive at runtime. A manifest with `scopes: [EventStore]` (wrong casing) passes the validator but the running DAPR sidecar denies access. Use explicit ordinal comparison: `if (-not ($Scopes | Where-Object { $_ -ceq $requiredScope })) { return $false }` or `[StringComparer]::Ordinal`.
- [x] [Review][Patch] `Get-YamlKind` returns only the first document's kind in a multi-document file [`deploy/validate-deployment.ps1:974` Test-EventStoreFrontedTopology] — A `topology.yaml` whose first `---` block is a different kind (Component, Configuration) is silently dropped by the topology file filter. Use `Split-YamlDocuments` already present in the codebase (line 783 vicinity) and check every document's kind.

#### Deferred

- [x] [Review][Defer] xUnit v3 partial cross-cutting audit [`tests/**/csproj`] — deferred, pending a dedicated cross-cutting xUnit v3 audit story that confirms all test fixtures' `IAsyncLifetime` returns `ValueTask` instead of `Task`.
- [x] [Review][Defer] `commandApiAppId: eventstore` flip has no migration shim [`deploy/dapr/tenants-integration.yaml:373`] — deferred, the pivot is the explicit Wave 2 contract and adopters are still onboarding. Add a migration note to the deferred-work pile for the Tenants integration release notes.
- [x] [Review][Defer] Topology check uses non-recursive `Get-ChildItem` [`deploy/validate-deployment.ps1:974`] — deferred, project convention places all DAPR yaml under flat `deploy/dapr/`. Revisit if a future deploy template introduces subdirectories.
- [x] [Review][Defer] Story 12.1 Deferred Gap row "Production trust-domain placeholders" claims `implemented-here` without a new validator assertion — deferred, paperwork. The `Test-AccessControl` trust-domain check is unchanged from Story 12.1; new templates use the env-var placeholder but no NEW test asserts the placeholder pattern. Either re-classify as `deferred-with-owner` or add a placeholder-pattern test.
- [x] [Review][Defer] No negative fixture for wildcard appId on the new EventStore caller matrix [`tests/Hexalith.Parties.DeployValidation.Tests/DeploymentValidationTests.cs`] — deferred, existing wildcard guard in `Test-AccessControl` still runs over the new caller list. Add a focused negative test only if the guard regresses.
...
- Keep package versions aligned with this repository: .NET SDK `10.0.103`, `net10.0`, Aspire `13.2.2`, Aspire Hosting Testing `13.2.1`, CommunityToolkit Aspire DAPR `13.0.0`, Dapr Client/AspNetCore `1.17.7`, Dapr Actors `1.16.1`, `xunit.v3` (test projects migrated from xUnit 2.9.3 per `sprint-change-proposal-2026-05-12-xunit-v3.md`), Shouldly `4.3.0`, NSubstitute `5.3.0`, and YamlDotNet `16.3.0`. The migration adds `<NoWarn>$(NoWarn);xUnit1051</NoWarn>` in `tests/Directory.Build.props` to suppress the v3 cancellation-token analyzer; the cancellation-token cleanup is tracked as a follow-up.
...
| `AppHostDaprTopologyValidationTests` | Static AppHost DAPR component checks | AC3, AC7, AC8 | Production validator did not enforce equivalent receiving-sidecar matrix | No changes to this file in this story; the equivalent production validator caller matrix is implemented in `deploy/validate-deployment.ps1` (`Test-AccessControl`) and exercised through `DeploymentValidationTests.cs` and `TenantsDeploymentValidationTests.cs`. Static AppHost-side coverage remains as-is. | None |
