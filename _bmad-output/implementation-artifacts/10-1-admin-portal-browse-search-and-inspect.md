...
- [x] Add Parties admin portal composition on top of Hexalith.FrontComposer (AC: 1, 8)
  ...
  - [~] Reuse FrontComposer shell services, navigation, density, DataGrid, query transport, command feedback, auth redirect, and localization seams where available. **Partial — deferred to Story 10-1.1 (D4 in pass-1 review).** Current implementation uses raw HTML elements; FrontComposer Fluent UI adoption is scaffolding-only.
...
- [x] Add admin authorization and tenant-context UX coverage (AC: 1, 4, 7)
  ...
  - [~] Use Hexalith.Tenants-provided tenant context and role state from the completed Epic 11 stories; do not infer authorization from the JWT tenant claim alone. **Partial — deferred to Story 10-1.1 (D2b in pass-1 review).** Component currently relies on host-supplied auth booleans (default `false`, fail-closed); direct integration with `Hexalith.Tenants` services is scaffolding-only.
...
- 2026-05-07 - BMad pass-2 code review applied. 5 decision-needed (D1 status casing, D2 search-mode filter UX, D3 scoped-id display, D4 Labels staleness, D5 dead-code services) + 40 patches (P1-P40) applied; 9 deferred to follow-up (W7-W15 in deferred-work.md); 12 dismissed as noise. Story 10-1.1 placeholder created for D2b/D3/D4 + D5 follow-up. Validation: `dotnet build Hexalith.Parties.slnx --configuration Release` clean (0 warnings, 0 errors); `dotnet test tests/Hexalith.Parties.AdminPortal.Tests --configuration Release --no-build` 25/25; client/contracts test suites 59 passed / 6 skipped and 42 passed / 15 skipped (skips are Story 10-2 GDPR scaffolds, out of scope).
...
- 2026-05-07 - BMad pass-2 code review: applied 5 decisions (D1-D5) + 40 patches (P1-P40); created Story 10-1.1 placeholder for deferred FrontComposer/Tenants/rich-search work; story moved review → done.
...
### Pass-2 Review Findings (2026-05-07)

Pass-2 BMad code review (2026-05-07). Three reviewers: Blind Hunter (adversarial, diff-only), Edge Case Hunter (path enumeration + project read), Acceptance Auditor (diff vs spec). Diff range `e0f854c~1..9b6d180` filtered to AdminPortal scope, 30 files, ~2.8k lines added. Pass-1 items D1/D2a/D5 + 37 patches verified — most applied honestly; the gaps below survived pass-1.
...
- [x] [Review][Decision][Resolved → Patch D5.1] **D5 — Are `AdminPortalPartyQueryService` and `PartiesAdminListCoordinator` retained dead code or active integration points?** — Resolution: keep as 10-1.1 scaffolding. Add `// TODO(Story 10-1.1): wire into PartiesAdminPortal.razor — currently scaffolding for FrontComposer integration` to both services and to `AdminPortalListState`. Downgrade the fitness tests `AdminPortal_DefinesScopedQueryServiceFailingClosed` and the coordinator-existence checks to explicitly mark themselves as scaffolding-presence guards (rename or add comment). — [src/Hexalith.Parties.AdminPortal/Services/AdminPortalPartyQueryService.cs; PartiesAdminListCoordinator.cs; AdminPortalListState.cs; tests/Hexalith.Parties.Contracts.Tests/AdminPortal/AdminPortalAuthorizationStateTests.cs:2531-2592]

#### Patch

- [x] [Review][Patch][Applied] **P1 — `_page` desynchronizes from server-returned page** — `ApplyRowsAsync` reads `page.TotalPages`/`page.TotalCount`/`page.Page` but never assigns `_page = page.Page`. Server clamping (page 50 → page 5) leaves `_page` at 50; Previous goes to 49 still out of range. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:447-465]
- [x] [Review][Patch][Applied] **P2 — `AdminPortalPartyQueryService.Dispose` crashes on second call and `ScopeCancellationToken` reads disposed CTS after `ResetForTenantSwitch`** — `Cancel()` after prior `Dispose()` throws `ObjectDisposedException`; the property getter races the swap. Add `_disposed` guard. [src/Hexalith.Parties.AdminPortal/Services/AdminPortalPartyQueryService.cs:962-981]
- [x] [Review][Patch][Applied] **P3 — Fitness test `AdminPortal_DoesNotShipParallelSpaArtifacts` scans `bin/Debug/`, not source** — `assemblyDirectory = Path.GetDirectoryName(portal.Location)` is the test bin folder; `package.json`/`vite.config.js` would never land there. Tautology that always passes. Resolve project source path via MSBuild metadata or `[CallerFilePath]`. [tests/Hexalith.Parties.Contracts.Tests/AdminPortal/AdminPortalReadOnlySurfaceTests.cs:2691-2707]
- [x] [Review][Patch][Applied] **P4 — Fitness test `AdminPortal_RoutesAreScopedToAdminPartiesPath` may pass with zero coverage** — reflection failure swallowed; if `routes` enumerates empty, the foreach is vacuous and the test passes without asserting a route exists. Add `routes.ShouldNotBeEmpty()` and replace forced `!` on `Template` with explicit guards. [tests/Hexalith.Parties.Contracts.Tests/AdminPortal/AdminPortalReadOnlySurfaceTests.cs:2769-2806]
- [x] [Review][Patch][Applied] **P5 — Fitness test `AdminPortal_DefinesScopedQueryServiceFailingClosed` verifies `IDisposable`, not Scoped lifetime** — test name promises lifetime guarantee; implementation only checks disposability. Verify the `ServiceDescriptor.Lifetime`. [tests/Hexalith.Parties.Contracts.Tests/AdminPortal/AdminPortalAuthorizationStateTests.cs:2572-2592]
- [x] [Review][Patch][Applied] **P6 — `AdminPortalQueryMetadata` constructor re-runs `BoundHeaderValue` after init setters already bounded** — idempotent but signals confused ownership of validation. Remove the re-bounding from the constructor body. [src/Hexalith.Parties.AdminPortal/Services/AdminPortalQueryMetadata.cs:1075-1098]
- [x] [Review][Patch][Applied] **P7 — `PartiesAdminPortalApiClient` constructor throws `InvalidOperationException` when `BaseAddress` not configured** — deferred runtime crash with no startup signal. Add `IValidateOptions<PartiesAdminPortalOptions>` with `.ValidateOnStart()` so missing config fails at host startup, not on first request. [src/Hexalith.Parties.AdminPortal/Extensions/PartiesAdminPortalServiceCollectionExtensions.cs:701-708; Services/PartiesAdminPortalApiClient.cs:1252-1256]
- [x] [Review][Patch][Applied] **P8 — Client filters `IsErased` rows but `TotalCount`/`TotalPages` come from server unfiltered** — UX shows `Page 1 of 5, 100 total` with only 18/20 rows; if server includes erased counts, the discrepancy leaks an erased-record signal. Trust server filtering or expose a `Filtered N of M` indicator. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:455-457]
- [x] [Review][Patch][Applied] **P9 — JSON shape mismatches surface as `Unknown` with no diagnostics** — `JsonException` is dropped. Preserve as `InnerException` of `AdminPortalQueryException` for log correlation. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs:1328-1331, 1360-1363]
- [x] [Review][Patch][Applied] **P10 — Null `Items`/`Results` from server causes `NullReferenceException` to escape uncaught** — `body.Results?.Items ?? Array.Empty<PartySearchResult>()` defensive guard, plus null-check on the envelope. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs:104-115]
- [x] [Review][Patch][Applied] **P11 — `204 No Content` / empty body on success treated as `Unknown` failure** — `ReadFromJsonAsync` returns null → throws `Unknown`. Detect 204 and treat as empty result. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs ReadPayloadAsync]
- [x] [Review][Patch][Applied] **P12 — `304 Not Modified` not handled** — also treated as `Unknown` because body is empty; conditional GET that should mean "unchanged" displays a load-failure to the user. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs SendAsync]
- [x] [Review][Patch][Applied] **P13 — Unexpected `Content-Type` (HTML captive portal) maps to `Unknown`** — should map to `AuthenticationRequired`. Detect non-JSON content type and route appropriately before `ReadFromJsonAsync`. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs SendAsync]
- [x] [Review][Patch][Applied] **P14 — `Retry-After` header on 429/503 ignored** — clicking Retry immediately just gets another 429. Surface the suggested delay or auto-throttle. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs failure mapping]
- [x] [Review][Patch][Applied] **P15 — `IOException`/`SocketException` outside `HttpRequestException` not caught** — propagates to Blazor circuit error handler. Catch base `IOException` and map to `TransientFailure`. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs catch blocks]
- [x] [Review][Patch][Applied] **P16 — `AuthenticationRequired` vs `TenantRequired` conflation** — backend uses `403 Forbidden` for missing tenant; client maps both 403 paths identically. Distinguish via response body or specific header so the user can choose to switch tenant vs escalate role. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs status-to-failure-kind mapping]
- [x] [Review][Patch][Applied] **P17 — `SelectPartyAsync` shows no loading indicator during in-flight detail fetch** — slow loads look indistinguishable from "no party selected". Show busy state in the detail panel. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:468-512, detail render]
- [x] [Review][Patch][Applied] **P18 — Late `SelectPartyAsync` callback after `Dispose` creates leaked CTS in `_detailCts`** — `SwapDetailCts` installs a fresh CTS post-dispose; never cancelled or disposed. Guard `SelectPartyAsync` with `if (_disposed) return;`. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:468-512]
- [x] [Review][Patch][Applied] **P19 — `_disposed` field not volatile** — late-completing tasks may not see the flush on ARM. Use `Volatile.Read`/`Volatile.Write` or `Interlocked` semantics. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:Dispose, SafeStateHasChangedAsync]
- [x] [Review][Patch][Applied] **P20 — `RecordingAdminPortalApiClient.SearchPartiesAsync` re-bounds `PageSize`** — masks real-component bugs; tests assert against the stub's bound, not the component's. Remove the bound from the stub. [tests/Hexalith.Parties.AdminPortal.Tests/Services/RecordingAdminPortalApiClient.cs:2180-2194]
- [x] [Review][Patch][Applied] **P21 — `AdminPortalQueryException.DefaultMessage` produces identical strings for `Validation` and `Unknown`** — operators reading logs cannot differentiate. Add a `Validation`-specific message. [src/Hexalith.Parties.AdminPortal/Services/AdminPortalQueryException.cs:1029-1032]
- [x] [Review][Patch][Applied] **P22 — Detail panel `_detailEmptyText` doesn't reflect unauthorized/missing-tenant intent** — UX matrix prescribes "Sign-in is required" / "Tenant context is unavailable" / "Access is denied" for those states; current code leaves it at "Select a party". Set `_detailEmptyText` per auth state in `OnParametersSetAsync`. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:325-344, 510]
- [x] [Review][Patch][Applied] **P23 — `@row.Type`, `@channel.Type`, `@identifier.Type`, `@consent.Purpose` render raw enum literals without translation** — localization gap. Use `EffectiveLabels.PartyTypeLabel(...)` etc. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:69, contact channels and identifiers blocks]
- [x] [Review][Patch][Applied] **P24 — `StatusLabel(row)` doesn't reflect `IsRestricted` in the row badge** — list shows raw Active/Inactive while detail shows full state. Align by including restricted state in the row badge. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor StatusLabel, ApplyRowsAsync]
- [x] [Review][Patch][Applied] **P25 — Focus loss after pagination/search/error retry** — keyboard users must Tab back from start. Add `FocusAsync` to a sensible target (first row on grid update, retry button on failure). [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor]
- [x] [Review][Patch][Applied] **P26 — `<button disabled>` for `DisplayNameMode` lacks `aria-disabled`** — inconsistent with Email/Identifier buttons which add it. Screen-reader announcement is degraded. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:46]
- [x] [Review][Patch][Applied] **P27 — Detail panel section IDs (`party-summary-heading`, etc.) are global** — would collide if two portals mount on same page. Prefix with a per-instance id. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor detail panel headings]
- [x] [Review][Patch][Applied] **P28 — `AdminPortalQueryFailureKind.Validation` 400/422 ProblemDetails body discarded** — user has no recourse. Surface field-level errors. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs failure mapping]
- [x] [Review][Patch][Applied] **P29 — Retry button only shown for `TransientFailure`** — `Unknown` (e.g., backend contract drift) and `Validation` are also retryable in spirit but offer no Retry. Extend Retry visibility. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor retry render]
- [x] [Review][Patch][Applied] **P30 — `200 OK` with `IsErased=true` (rebuilding projection) doesn't remove the row from `_rows`** — the 410 path does. Align both branches; surface `StaleDataAge` on the erased detail outcome. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:SelectPartyAsync erased branch]
- [x] [Review][Patch][Applied] **P31 — `FormatDate` returns empty string for `default(DateTimeOffset)`** — grid shows blank cells indistinguishable from layout glitch. Use placeholder ("—") or hide column. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:FormatDate]
- [x] [Review][Patch][Applied] **P32 — `PortalFilters.Type` doesn't trim whitespace before `Enum.TryParse`** — copy-pasted "  Person  " is silently dropped. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor:PortalFilters]
- [x] [Review][Patch][Applied] **P33 — Story task line 52 `[x] Reuse FrontComposer shell services...` is checked despite work deferred to D4 in 10-1.1** — downgrade to `[partial]` or uncheck with a defer marker. [_bmad-output/implementation-artifacts/10-1-admin-portal-browse-search-and-inspect.md:52]
- [x] [Review][Patch][Applied] **P34 — Story task line 99 `[x] Use Hexalith.Tenants-provided tenant context and role state...` is checked despite work deferred to D2b in 10-1.1** — downgrade or uncheck. [_bmad-output/implementation-artifacts/10-1-admin-portal-browse-search-and-inspect.md:99]
- [x] [Review][Patch][Applied] **P35 — Add D2b/D3/D4 from pass-1 to `_bmad-output/implementation-artifacts/deferred-work.md`** — currently only mentioned inside the story spec; project-level tracking missing. Future readers of `deferred-work.md` will not see the AC3/AC7/AC8 partial-met posture. [_bmad-output/implementation-artifacts/deferred-work.md]
- [x] [Review][Patch][Applied] **P36 — Story 10-1.1 referenced four times but does not exist as a sprint-status entry or file** — either create the placeholder story or remove the references and route deferrals through `deferred-work.md` only. [_bmad-output/implementation-artifacts/sprint-status.yaml; _bmad-output/implementation-artifacts/]
- [x] [Review][Patch][Applied] **P37 — `PartySearchResponse.ScoreMetadata`/`SourceMetadata` discarded by `AdminPortalSearchResponse`** — silent drop of scoring/source metadata. Either model them for future surfacing or document the intentional drop with a `// TODO: 10-1.1` marker. [src/Hexalith.Parties.AdminPortal/Services/AdminPortalSearchResponse.cs]
- [x] [Review][Patch][Applied] **P38 — Add component-level test for cross-tenant scoped id rejection (Task line 100 partial-met)** — current transport-only leakage check exists; no end-to-end "scoped id from another tenant rendered nothing" assertion. [tests/Hexalith.Parties.AdminPortal.Tests/Components/PartiesAdminPortalComponentTests.cs]
- [x] [Review][Patch][Applied] **P39 — Add focus/keyboard-navigation tests claimed by pass-1 P30** — aria-label assertions are insufficient; the task line 110 `[x]` lacks evidence of `KeyDown`/`Tab`/`FocusAsync` exercises. [tests/Hexalith.Parties.AdminPortal.Tests/Components/PartiesAdminPortalComponentTests.cs]
- [x] [Review][Patch][Applied] **P40 — Wire-status casing alignment (depends on D1)** — once D1 is decided, change either the converter naming policy on the client or the comparison literal in `IsLocalOnlySearch` to match the agreed wire form. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs:19; AdminPortalQueryMetadata.cs IsLocalOnlySearch]

#### Deferred

- [x] [Review][Defer] **W7 — `PageSize` negative coerced silently to default** — `BoundPageSize(-1)` returns 20 without warning the caller. Defensive only; current `<select>` only emits valid values. [src/Hexalith.Parties.AdminPortal/Services/AdminPortalQueryBounds.cs]
- [x] [Review][Defer] **W8 — Zero-width unicode chars in query (`U+200B`) routed to search instead of list** — `IsNullOrWhiteSpace` does not recognize them; backend's `IsEffectivelyEmptyQuery` does and returns LocalOnly empty. UI reports "Display-name search only" status when the user typed nothing visible. Edge case; defensive normalization. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor LoadPageAsync routing]
- [x] [Review][Defer] **W9 — Long scoped party ids may overflow URL length** — no client-side guard. Pre-existing pattern; ASP.NET routing typically limits at proxy layer. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs GetPartyAsync]
- [x] [Review][Defer] **W10 — `PartiesAdminPortalOptions.ApiBaseAddress` setter publicly mutable** — a misconfigured caller could change the base address mid-app. No concrete defect today. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalOptions.cs]
- [x] [Review][Defer] **W11 — `@key="row.Id"` lacks defensive uniquification on duplicate ids** — backend filters by dictionary key, but a malformed projection could deliver duplicates and Blazor would throw. Pre-existing pattern. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor row binding]
- [x] [Review][Defer] **W12 — `DateTimeOffset.ToString("g", CurrentCulture)` Blazor Server vs browser culture mismatch** — server-side rendering uses server culture; user's browser culture is not honored. Full localization deferred to FrontComposer integration in Story 10-1.1. [src/Hexalith.Parties.AdminPortal/Components/PartiesAdminPortal.razor FormatDate]
- [x] [Review][Defer] **W13 — `PartiesAdminListCoordinator.State` lacks memory barrier** — moot because the type is dead code (see D5); revisit if D5 wires the type. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminListCoordinator.cs:1199-1209]
- [x] [Review][Defer] **W14 — `AddDate` produces verbose URL-encoded dates (~250 chars when all four set)** — defensive only; not a bug. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs AddDate]
- [x] [Review][Defer] **W15 — `Search` request with empty/whitespace `Query` reachable only via the public API surface** — UI routes whitespace-only queries to LIST; the search-with-empty-q path is unreachable from the component but not enforced at API client level. Defensive. [src/Hexalith.Parties.AdminPortal/Services/PartiesAdminPortalApiClient.cs SearchPartiesAsync]

#### Dismissed (12)

- `OnParametersSetAsync` race with in-flight `SelectPartyAsync` — handled by version checks (`_listVersion`/`_detailVersion`).
- `HttpRequestException.StatusCode` null on raw network errors — handled (kind forced to `TransientFailure`).
- `_lastFailure` retained across tenant change — handled by `ResetVisibleState`.
- Internal `HttpClient` 100s timeout `TaskCanceledException` mapping — correctly handled via `when (!cancellationToken.IsCancellationRequested)` guard.
- `createdBefore`/`modifiedBefore` filters not exposed — explicitly out of scope for this story.
- `GetPartyAsync` URL-encoding `:` as `%3A` — ASP.NET routing decodes before binding; verified-handled. (Contract test lock added as P38.)
- `ContextKey` null vs empty signature collision — Acceptance Auditor withdrew after re-read; canonicalization to `<null>` is correct.
- Empty `aria-label` first render — initial value `"Loading parties"` is non-empty.
- Pass-1 P5 (uncaught HttpRequestException/JsonException/TaskCanceledException) — verified applied.
- Pass-1 P11 (ObjectDisposedException on InvokeAsync) — verified applied.
- Pass-1 P25 (async stale-response suppression) — verified applied via `TaskCompletionSource`-based test.
- Pass-1 D2a (auth defaults flipped to `false`) — verified applied at `Components/PartiesAdminPortal.razor:283-289`.
