...
- [x] [Review][Defer] EventStore submodule change conflicts with Story 12.4 scope [`Hexalith.EventStore/src/Hexalith.EventStore.Client/Aggregates/EventStoreAggregate.cs:95`] — deferred to a separate EventStore story/patch per 2026-05-11 review decision. Story 12.4's technical constraints say "Do not edit the `Hexalith.EventStore` submodule in this story" and to record a separate EventStore blocker if platform behavior must change.
- [x] [Review][Defer] Async derived DomainResult handlers are accepted but cannot dispatch [`Hexalith.EventStore/src/Hexalith.EventStore.Client/Aggregates/EventStoreAggregate.cs:95`] — deferred to a separate EventStore story/patch per 2026-05-11 review decision. The discovery predicate accepts any `Task<T>` where `T : DomainResult`, such as `Task<CompositeCommandResult>`, but `DispatchCommandAsync` still only matches `Task<DomainResult>` at line 141. Because `Task<T>` is invariant, an accepted async handler returning a derived result falls through to "unexpected type" at runtime.
