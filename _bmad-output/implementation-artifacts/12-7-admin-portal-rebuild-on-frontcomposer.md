...
### Review Findings (Group A — Core services, 2026-05-13)

- [x] [Review][Decision] D1: IsContractUnavailable sentinel matches `AdminPortalGdprOutcome.ContractUnavailable.ToString()` literally — if the server emits a different title string for 501, the mapping silently falls through to `TransientFailure` or `Unknown`, hiding the contract-unavailable state. Options: (a) map HTTP 501 unconditionally to ContractUnavailable regardless of title; (b) keep the heuristic but validate the sentinel string against the Story 12.5 wire format in a test. [PartiesAdminPortalApiClient.cs:IsContractUnavailable]
- [x] [Review][Decision] D2: IQueryService fallback path passes `ProjectionActorType: _options.DetailProjectionActorType` (e.g., `"PartyDetailProjectionActor"`) to FrontComposer QueryRequest — if EventStore resolves this to a DAPR projection actor, it violates AC3 ("no projection actor reads remain"). Options: (a) accept the abstraction as sufficient (FrontComposer owns the resolution, not AdminPortal); (b) strip ProjectionActorType from the fallback QueryRequest and rely on server-side defaults. [PartiesAdminPortalApiClient.cs:GetPartyAsync, PartiesAdminPortalOptions.cs:DetailProjectionActorType]
- [x] [Review][Patch] P1: `ExecuteGdprCommandAsync` and `ExecuteGdprQueryAsync` catch `OperationCanceledException` and return `TransientFailure`/rethrow as `AdminPortalQueryException(TransientFailure)` even when the caller's token was signalled — legitimate cancellations are indistinguishable from transient failures. Fix: add `catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { throw; }` before the broad catch; requires adding a `CancellationToken` parameter to both private methods. [PartiesAdminPortalApiClient.cs:ExecuteGdprCommandAsync, ExecuteGdprQueryAsync]
- [x] [Review][Patch] P2: `ExecuteGdprCommandAsync` returns `new AdminPortalGdprCommandResult(MapGdprOutcome(ex), ex.CorrelationId, ex.Detail)` on `PartiesClientException` — `ex.Detail` is a raw ProblemDetails detail string that may contain party names, identifiers, or tenant membership, violating AC8/AC9. Fix: use `null` for `Detail` in the command result (the correlation ID plus the outcome code are sufficient bounded feedback). [PartiesAdminPortalApiClient.cs:ExecuteGdprCommandAsync]
- [x] [Review][Patch] P3: `AdminPortalGdprStateCoordinator.CanMutateParty` returns `true` for `MissingToken`, `MissingTenant`, `Forbidden`, and `DomainRejected` — auth-failure and domain-rejection states should block mutations to satisfy the fail-closed requirement of AC9. Fix: add these states to the `is not (...)` exclusion list. [AdminPortalGdprStateCoordinator.cs:CanMutateParty]
- [x] [Review][Patch] P4: `Hexalith.Parties.AdminPortal.csproj` references FrontComposer Shell with `AdditionalProperties="TreatWarningsAsErrors=false"`, silencing all warnings from the shell (including nullability and new analyzer warnings), not just the two known pre-existing ones (`CS9113`, `CS0162`). Fix: replace with `<NoWarn>CS9113;CS0162</NoWarn>` in a `<ProjectReference>` property group or a `<Directory.Build.targets>` condition scoped to that reference. [Hexalith.Parties.AdminPortal.csproj]
- [x] [Review][Patch] P5: `ToPage<T>` computes `TotalPages = (int)Math.Ceiling(result.TotalCount / (double)pageSize)` without guarding against `TotalCount < 0`; a malformed upstream response produces a negative `TotalPages` that propagates to components. Fix: clamp to `Math.Max(0, (int)Math.Ceiling(...))`. [PartiesAdminPortalApiClient.cs:ToPage]
- [x] [Review][Patch] P6: `ComputeBoundedSkip` throws `AdminPortalQueryFailureKind.Validation` with message "Requested page exceeds the addressable range" when `page < 1` (e.g., `page=0` → `skip = -pageSize < 0`); the message is misleading. Fix: add an explicit `if (page < 1) throw new AdminPortalQueryException(Validation, validationDetail: "Page number must be >= 1.")` before the multiplication. [PartiesAdminPortalApiClient.cs:ComputeBoundedSkip]
- [x] [Review][Defer] W1: `ContainsTenant` inspects raw `ProblemDetails.Title`, `Detail`, and `GlobalErrors` text to discriminate `TenantRequired` vs `Forbidden` on 403 responses — fragile string heuristic that may read PII-bearing detail text; pre-existing pattern carried over from typed-client mapping. Defer to a future story that defines a structured tenant-error indicator in the EventStore ProblemDetails contract. [PartiesAdminPortalApiClient.cs:ContainsTenant, IsTenantProblem] — deferred, pre-existing
- [x] [Review][Defer] W2: `CanMutateParty` uses a blocklist; new `AdminPortalGdprOperationState` values added in future will default to mutable — an allowlist would be safer for an erasure guard. Defer: no active bug, but future state additions need to be reviewed against mutability. [AdminPortalGdprStateCoordinator.cs:CanMutateParty] — deferred, pre-existing
- [x] [Review][Defer] W3: `PartiesAdminPortalOptionsValidator.Validate` creates a DI scope at `ValidateOnStart` startup time; if `IPartiesQueryClient` requires `IHttpContextAccessor` for construction, the scope resolution throws and the host fails to start — known limitation of startup-time DI probing. Defer: fail-fast at startup is arguably correct; revisit if host startup failures are observed in practice. [PartiesAdminPortalOptionsValidator.cs:Validate] — deferred, pre-existing
- [x] [Review][Defer] W4: Fitness test `AdminPortalSource_DoesNotContainRetiredPartiesRestReads` does not scan for `ProjectionActorType`, `PartyDetailProjectionActor`, or DAPR actor patterns in AdminPortal source — the constraint against projection actor reads is unverified for the IQueryService fallback path option value. Defer to a future fitness test enhancement. [tests/Hexalith.Parties.AdminPortal.Tests] — deferred, pre-existing
