...
- [x] Compose Hexalith.Tenants into the Parties AppHost topology (AC: 1)
  ...
  - [x] Ensure the Tenants resource appears with a stable Aspire resource name, preferably `tenants`, and the existing `parties` resource keeps its current DAPR app id as the EventStore command gateway hosting Parties module handlers.
  - [x] Wire the `parties` EventStore command gateway to the Tenants resource with `WithReference(...)` and `WaitFor(...)` or an equivalent health-gated dependency, without breaking the existing EventStore topology created by `AddHexalithParties`.
...
- [x] Align local development bootstrap/configuration (AC: 2, 3)
  ...
  - [x] Add a Parties-side tenant integration configuration section, for example `Tenants:Enabled`, `Tenants:ServiceName`, `Tenants:CommandApiAppId`, `Tenants:PubSubName`, and `Tenants:TopicName`, matching the actual options consumed by the code. Keep `Tenants:CommandApiAppId` pointed at `parties` when it identifies the EventStore command gateway.
...
- [x] Add focused tests and validation (AC: 1, 3, 4)
  - [x] Add AppHost or configuration tests that prove the Tenants project/reference is present and the `parties` EventStore command gateway dependency is wired.
...
Client applications send tenant-scoped commands to the EventStore command API identified by DAPR app id `parties`. EventStore owns the tenant access gate: it checks whether the user can access the target tenant, then routes the command to the Parties module handler. Parties owns command/domain validation and event production after the command has passed through that gateway.

For this story, `parties` is therefore intentional: it is the EventStore command gateway hosting/routing Parties module handlers, not a Tenants service resource and not a reason to point Tenants command routing at `tenants`. The `tenants` resource is the Tenants service/topology resource that supports tenant lifecycle and access decisions. Story 11.1 must preserve this boundary and must not introduce direct client-to-Parties command ingress or duplicate tenant-access authorization inside Parties.
...
- Current state: creates the distributed app builder; resolves `DaprComponents/accesscontrol.yaml` from the working directory with a base-directory fallback; adds `parties`; calls `builder.AddHexalithParties(commandApi, accessControlConfigPath)`; optionally wires Keycloak to `parties`; configures publish targets; then runs the AppHost.
- Story change: add the Tenants project resource, call `AddHexalithTenants`, and wire the `parties` EventStore command gateway to Tenants. Keep existing Keycloak and publish-target behavior.
- Preserve: `parties` DAPR app id, existing access-control validation, existing EventStore/Parties topology, Keycloak fallback behavior, and publish-target logic.

`src/Hexalith.Parties.AppHost/Hexalith.Parties.AppHost.csproj`

- Current state: uses `Aspire.AppHost.Sdk/13.1.2`, references `Hexalith.Parties.Aspire` with `IsAspireProjectResource="false"` and `Hexalith.Parties`, and copies `DaprComponents/**/*` and `KeycloakRealms/**/*`.
...
- `HexalithTenantsOptions` defaults: `PubSubName = "pubsub"`, `TopicName = "system.tenants.events"`, and `CommandApiAppId = "parties"`. In the Parties topology, `CommandApiAppId = "parties"` is intentional when it points to the EventStore command gateway that performs tenant access checks before dispatching to Parties module handlers; do not change it to `tenants` merely because the Tenants service resource uses app id `tenants`. [Source: Hexalith.Tenants/src/Hexalith.Tenants.Client/Configuration/HexalithTenantsOptions.cs]
...
- Do not bypass the EventStore command API for tenant-scoped commands. The intended command path is `client -> parties/EventStore -> tenant access validation -> Parties module handler`.
...
- If changing DAPR component scopes or access-control policies, verify both app ids: EventStore command gateway `parties` and Tenants service `tenants`.
...
- Run affected tests:
  - `dotnet test tests/Hexalith.Parties.Tests/Hexalith.Parties.Tests.csproj --configuration Release`
...
- Aspire AppHost resource names participate in service discovery and visible topology. Keep names stable (`parties`, `tenants`) so future configuration and docs can rely on them. Source: https://learn.microsoft.com/en-us/dotnet/aspire/fundamentals/app-host-overview
...
- Tenants-specific runtime integration for CommandApi belongs under `src/Hexalith.Parties`, likely `Configuration/`, `HealthChecks/`, or `Extensions/` depending on the final design.
- Reusable Parties hosting abstractions belong in `src/Hexalith.Parties.Aspire`.
- Tests should stay in the nearest existing test project: CommandApi validation/health checks in `tests/Hexalith.Parties.Tests`, AppHost topology tests in an AppHost-focused test project if one is introduced.
...
- `dotnet test tests/Hexalith.Parties.Tests/Hexalith.Parties.Tests.csproj --configuration Release --filter "FullyQualifiedName~TenantIntegrationOptionsValidatorTests|FullyQualifiedName~TenantsIntegrationHealthCheckTests|FullyQualifiedName~AppHostTenantsTopologyTests"` — passed, 10/10.
- `dotnet test tests/Hexalith.Parties.Tests/Hexalith.Parties.Tests.csproj --configuration Release --filter "FullyQualifiedName~HealthEndpointIntegrationTests"` — passed, 12/12.
...
- `dotnet test tests/Hexalith.Parties.Tests/Hexalith.Parties.Tests.csproj --configuration Release --filter "FullyQualifiedName!~SemanticSearchPerformanceBenchmarkTests"` — passed, 396/396.
- Full `Hexalith.Parties.Tests` run: 399/400 passed; `SemanticSearchPerformanceBenchmarkTests.Search_100KEntries_ExactMatch_CompletesWithin500ms` missed the 500ms threshold in full-suite runs (509-514ms) and passed when rerun alone once. No Tenants/AppHost functional failures remained.
...
- Findings summary:
  ...
  - AppHost topology guidance should make `tenants`, `parties`, and `eventstore` app-id/resource boundaries explicit so development does not replace the EventStore command gateway or overbuild later Story 11.2/11.3 responsibilities.
...
- Findings deferred:
  ...
  - Confirm whether `WaitFor(tenants)` is required for `parties` startup or only for AppHost topology visibility.
...
3. Create a default tenant and assign the sample user a Parties-permitting role using the Tenants Command API (`parties` DAPR app id, exposed by the EventStore command gateway). Issue the canonical Tenants commands as that admin:
...
- Composed Hexalith.Tenants into the Parties AppHost with stable `tenants` resource name, reused `AddHexalithTenants`, shared the Tenants DAPR state/pubsub resources with Parties, and added `WithReference(...).WaitFor(...)` from `parties` to the Tenants project resource.
...
- src/Hexalith.Parties/Configuration/TenantIntegrationOptions.cs
- src/Hexalith.Parties/Extensions/PartiesServiceCollectionExtensions.cs
- src/Hexalith.Parties/HealthChecks/PartiesHealthCheckExtensions.cs
- src/Hexalith.Parties/HealthChecks/TenantsIntegrationHealthCheck.cs
- src/Hexalith.Parties/appsettings.Development.json
- src/Hexalith.Parties/appsettings.json
- tests/Hexalith.Parties.Tests/Configuration/TenantIntegrationOptionsValidatorTests.cs
...
- tests/Hexalith.Parties.Tests/HealthChecks/TenantsIntegrationHealthCheckTests.cs
...
- [x] [Review][Patch] HttpClient lacks bounded request timeout in DaprTenantsReadinessProbe — Fixed: named client `tenants-readiness-probe` registered with `Timeout = TimeSpan.FromSeconds(2)`; probe uses `CreateClient(HttpClientName)`. [src/Hexalith.Parties/Extensions/PartiesServiceCollectionExtensions.cs, HealthChecks/TenantsIntegrationHealthCheck.cs]
- [x] [Review][Patch] Catch swallows `OperationCanceledException` and reports it as a check failure — Fixed: added `catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { throw; }` so graceful shutdown does not flap `/ready`. [src/Hexalith.Parties/HealthChecks/TenantsIntegrationHealthCheck.cs]
...
- [x] [Review][Patch] No `/ready=503` integration test for unavailable Tenants probe — Fixed: replaced fixture-level `HealthyTenantsReadinessProbe` with `SwitchableTenantsReadinessProbe`; added `ReadyEndpoint_TenantsIntegrationUnreachable_Returns503Async` asserting both the 503 response and the `tenants-integration` check status in the JSON payload. [tests/Hexalith.Parties.Tests/HealthChecks/HealthEndpointIntegrationTests.cs]
- [x] [Review][Defer] AppHost env-vars duplicate `appsettings.json` Tenants values for `parties` [src/Hexalith.Parties.AppHost/Program.cs:48-52] — env-var precedence makes JSON values dead under AppHost; future maintenance trap. Low priority; pre-existing pattern in the host.
- [x] [Review][Defer] `ValidateOnStart` eagerness not asserted by any test [src/Hexalith.Parties/Extensions/PartiesServiceCollectionExtensions.cs:79-82] — likely fires correctly under modern host, but no failing-startup test exists; if validation actually fires lazily, missing config produces a 500 from `/ready` rather than a startup crash. Add a startup-fail integration test in a hardening pass.
