...
- **Consequence:**
  ...
  - Sibling-submodule service projects are composed as `AddProject<…>` Aspire resources (Story 9.3 closed the gap for `Hexalith.Memories.Server`; FrontComposer is carved out to Story 9.4 pending an upstream deployable host project). Infrastructure dependencies (Keycloak, Redis) are composed by the AppHost and emitted by aspirate where translation is possible; resource types aspirate cannot translate deterministically (Keycloak admin credential + realm import, etc.) ship as hand-authored carve-outs under `deploy/k8s/<resource>/` with the byte-determinism contract scoped to exclude those subfolders only (Story 9.3 ADRs 9.3-2/9.3-3/9.3-4 document the in-scope sibling-submodule composition + the path-b carve-out boundary)
- **Affects:** `src/Hexalith.Parties.AppHost` (must remain aspirate-compatible — no Aspire features aspirate cannot translate), `deploy/k8s/` (generated artifact directory + hand-authored carve-out subfolders preserved by `publish.ps1`), `tests/Hexalith.Parties.DeployValidation.Tests` (extended to lint generated manifests + new Story 9.3 categories: `K8sTopology-MissingService`, `K8sSecret-JwtSigningKeyLiteral`, `K8sDapr-ResiliencyCrdSchemaDrift`), getting-started documentation (Story 3.7 + Story 9.3 pod-count update), Story 3.6 (Aspire local run remains valid — additive path)

**D-K8s-2 — Zot Registry as Image Substrate with Dedicated `parties-publisher` Build Account**

- **Decision:** Container images for all AppHost-composed services are built via the .NET SDK Container Publish target (invoked by aspirate when `--skip-build` is not passed), tagged with the MinVer-resolved version per build, and pushed to the Zot registry at `registry.hexalith.com` (cluster-side `project-zot/zot-linux-amd64` instance; htpasswd auth; nginx Ingress terminating HTTPS). Both operator-workstation publishes AND CI use a dedicated `parties-publisher` account in the Zot `builders` group — NOT the human-operator credentials (`jpiquot`, `qdassivignon` in `admins`). The `zot-pull-secret` Secret in `hexalith-parties` is bootstrapped by `deploy/k8s/publish.ps1` from the operator's `~/.docker/config.json` entry (Path B emission — the `auths` block is re-emitted wholesale without decoding the credential)
- **Rejected:**
  - Drop `--skip-build` in-place in `regen.ps1` and keep both scripts — the local-cluster regex allowlist in `deploy-local.ps1` still rejected the real cluster context, and the two-script UX persisted
  - Wrap the legacy scripts in a publish wrapper — the underlying scripts still failed to run on the real cluster
  - Use Aspirate's `AddKubernetesEnvironment` publisher instead of aspirate generate — Story 9.1 ADR D-K8s pins aspirate as the generator
  - Push using the operator's personal `admins`-group credential — couples pull-secret lifetime to operator's docker login, gives the kubelet push rights, stamps operator identity into every cluster Secret
  - Separate pull-only `zot-puller` account — adds a second cluster-side user-management surface for marginal least-privilege gain in MVP scope
  - Docker credential helper (`credsStore` / `credHelpers`) support — explicit Out of Scope; ~200 LoC + per-OS detection postponed to post-MVP
...
- **Rationale anchor (Story 9.1 v2 AC6 substring contract):** Path B sources credentials from the operator's `auths["registry.hexalith.com"]` entry which is never decoded and never echoed through the publish pipeline; `publish.ps1` exits 6 on `credsStore` / `credHelpers` indirection, and the approach keeps the credential surface minimal and auditable versus implementing a docker credential resolver in PowerShell.
- **Consequence:**
  ...
  - Story 9.1 AC1 byte-identical regen contract is **superseded on the image-tag line only** — non-image lines remain byte-stable per commit
...
- **Affects:** `deploy/k8s/publish.ps1` (new — subsumes `regen.ps1` + `deploy-local.ps1`), `deploy/k8s/teardown.ps1` (renamed from `teardown-local.ps1` — mandatory `-ConfirmContext` per ADR D-K8s-3), `deploy/validate-deployment.ps1` (`K8sWorkload-MissingImagePullSecret` category added), `tests/Hexalith.Parties.DeployValidation.Tests/K8sManifestPublishTests.cs` (new — 11 deploy-lane tests + 2 trait-gated live-cluster tests), `docs/deployment-guide.md` (Zot credentials subsection), `docs/getting-started.md` Step 1b (renamed Publish to a Kubernetes Cluster). (Wording anchor added per Story 9.1 v2 AC6/AC7 verification — minimal-diff to preserve mechanical grep contract.)
...
- **Rationale anchor (Story 9.1 v2 AC7 substring contract):** The gate compares `-ConfirmContext` against the value returned by `kubectl config current-context` (for the target cluster `kubernetes-admin@cluster.local` and any other operator-owned context); on mismatch the script does not echo cluster URL / CA / token bytes, exits 2 with the `expected '<arg>', got '<active>'` mismatch message, and the active context name is echoed once for auditability.
- **Consequence:**
