...
- **FR31a:** Deployment artifacts are generated from the Aspire AppHost via aspirate (aspir8), keeping the Aspire model as the single source of truth for the Parties + sibling-submodule service graph. Story 9.3 made FR31a **enumerative**: the topology comprises `eventstore`, `eventstore-admin`, `eventstore-admin-ui`, `parties`, `parties-mcp`, `tenants`, `memories` (composed in-cluster — no configurable external HTTP escape hatch; ADR 9.3-2), plus the infrastructure dependencies `keycloak` (hand-authored carve-out under `deploy/k8s/keycloak/` per ADR 9.3-3, path b) and `redis` (hand-authored backing store under `deploy/k8s/redis/`). FrontComposer is a deferred slot — Story 9.4 (`9-4-frontcomposer-deployable-host`) carries the upstream FrontComposer change needed to compose it as `AddProject<…>`. The aspirate-emitted manifests under `deploy/k8s/<app-id>/` retain a byte-determinism contract; hand-authored carve-outs (`keycloak`, `redis`) are excluded from byte-determinism on those subfolders only and preserved by `regen.ps1`'s `$PreservedNames` across regens
