...
- **FR31a:** Kubernetes manifests AND container images for the Parties topology can be generated and published from the Aspire AppHost via aspirate, with images pushed to the project's authoritative OCI registry (Zot at `registry.hexalith.com` for the in-MVP platform; ADR 9.5-1). Images carry the MinVer-resolved version as their tag and are immutable per commit. The topology is **enumerative** (Story 9.3): `eventstore`, `eventstore-admin`, `eventstore-admin-ui`, `parties`, `parties-mcp`, `tenants`, `memories` (composed in-cluster — no configurable external HTTP escape hatch; ADR 9.3-2), plus the infrastructure dependencies `keycloak` (hand-authored carve-out under `deploy/k8s/keycloak/` per ADR 9.3-3, path b) and `redis` (hand-authored backing store under `deploy/k8s/redis/`). FrontComposer is a deferred slot — Story 9.4 (`9-4-frontcomposer-deployable-host`) carries the upstream FrontComposer change needed to compose it as `AddProject<…>`. The aspirate-emitted manifests under `deploy/k8s/<app-id>/` retain a deterministic-per-commit contract (non-image lines are byte-stable; image-tag lines vary per commit per MinVer — Story 9.5 AC4 supersedes the prior byte-identical contract on the image line only); hand-authored carve-outs (`keycloak`, `redis`) are excluded from determinism on those subfolders only and preserved by `publish.ps1`'s `$PreservedNames` across publish runs
